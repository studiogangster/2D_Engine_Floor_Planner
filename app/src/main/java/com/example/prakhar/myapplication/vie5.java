 ...
 */
public class vie5 extends View {
...
drawPolygons();
    ...
    public vie5(Context context) {
        super(context);
     init();
    }

    public vie5(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public vie5(Context context, AttributeSet attrs, int defStyleAttr) {
  ...
  public  void scaleBy(float x , float y , float a , float b)
    {
        ...
        w = (getWidth()/2/scaleX)-originX;


        h = (getHeight()/2/scaleY)-originY;
...
Log.d("scalinghdas", originX + " " + originY);
        ...
        canvas.scale(x, y, originX, originY);

    //    translateBy((int) (((originX/x) +(getWidth()/2)/x)), (int)((originY/y)+(getHeight()/2/y)));
        translateBy((int) (polygons.get(0).points.get(1).x/scaleX ), (int)(polygons.get(0).points.get(1).y/scaleY+(getWidth()/2/scaleY)));
         drawPolygons();
...
//matrix.setScale(1.2f,1.2f,poly.Centre.x , poly.Centre.y);
            ...
            for(Polygon poli : polygons) {
                ...
                for (Point p : points) {

                    {
                        ...
                        canvas.drawText(""+  (p.x-minX), p.x, maxY +50+ 90, paint);
                        canvas.drawText(""+ (p.x-minX), p.x,  minY-90-30-50, paint);
  ...
  public  void addPolygon()
    {
      ...
      poi.add(new Point(MminX,MmaxY));
        ...
        poi.add(new Point(MminX +600,MmaxY+600));

        poi.add(new Point(MminX ,MmaxY+600));
        ...
        translateBy((int) (-originX + 0), (int) (-originY));
