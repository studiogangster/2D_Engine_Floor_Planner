 ...
 */
public class vietouchverified extends View {
...
Log.d("WHile Rotating" , " " + Centre.x +  " " + Centre.y);
...
Context con;
    public vietouchverified(Context context) {
        super(context);
     init(context);
    }

    public vietouchverified(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public vietouchverified(Context context, AttributeSet attrs, int defStyleAttr) {
...
for(int i=0;i<poly.points.size();i++)
{
   ...
   String dist =  String.format("%.02f",Math.sqrt(Math.pow(b.x - a.x, 2) + Math.pow(b.y - a.y, 2)));
    canvas.drawTextOnPath(""+dist, ph , 0,-50,paint);
  ...
  public  void addPolygon()
    {
      ...
      poi.add(new Point(MminX, MmaxY + 600));
        ...
        poi.add(new Point(MminX + 600, MmaxY + 600 + 600));

        poi.add(new Point(MminX ,MmaxY+600+600));

Polygon pooo = new Polygon(poi, CNTR);

  //      pooo.translateXY(-originX,-originY);
//pooo.rotateBy(Math.PI/4);
        polygons.add(pooo);
